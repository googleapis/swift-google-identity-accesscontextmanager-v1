...
Add `swift-google-identity-accesscontextmanager-v1` as a package dependency:

```bash
swift package add-dependency https://github.com/googleapis/swift-google-identity-accesscontextmanager-v1.git --from 0.1.0-preview
```

Then add `GoogleIdentityAccessContextManagerV1` to your target's dependencies:

```bash
swift package add-target-dependency GoogleIdentityAccessContextManagerV1 <target-name> --package swift-google-identity-accesscontextmanager-v1
```
